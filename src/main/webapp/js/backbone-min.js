(function() {
	...
	h.extend(d.prototype, o,
			{
				...
				changedAttributes : function(t) {
					if (!t)
						return this.hasChanged() ? h.clone(this.changed)
								: false;
					var e, i = false;
					var r = this._changing ? this._previousAttributes
							: this.attributes;
					for ( var s in t) {
				...
				url : function() {
					var t = h.result(this, "urlRoot")
							|| h.result(this.collection, "url") || U();
					if (this.isNew())
						return t;
	...
	var y = [ "forEach", "each", "map", "collect", "reduce", "foldl", "inject",
			"reduceRight", "foldr", "find", "detect", "filter", "select",
			"reject", "every", "all", "some", "any", "include", "contains",
			"invoke", "max", "min", "toArray", "size", "first", "head", "take",
			"initial", "rest", "tail", "drop", "last", "without", "difference",
	...
	var b = /^(\S+)\s*(.*)$/;
	var x = [ "model", "collection", "el", "id", "attributes", "className",
			"tagName", "events" ];
	...
	var E = typeof window !== "undefined" && !!window.ActiveXObject
	...
	h.extend(k.prototype, o, {
		...
		_routeToRegExp : function(t) {
			t = t.replace(A, "\\$&").replace(S, "(?:$1)?").replace($,
					function(t, e) {
						return e ? t : "([^/]+)"
					}).replace(H, "(.*?)");
			return new RegExp("^" + t + "$")
	...
	h.extend(I.prototype, o,
			{
				...
				start : function(t) {
					...
					var r = P.exec(navigator.userAgent.toLowerCase())
							&& (!i || i <= 7);
				...
				stop : function() {
					a.$(window).off("popstate", this.checkUrl).off(
				...
				navigate : function(t, e) {
					...
					if (this._hasPushState) {
						...
						if (this.iframe
								&& t !== this.getFragment(this
										.getHash(this.iframe))) {
							if (!e.replace)
								this.iframe.document.open().close();
							this
									._updateHash(this.iframe.location, t,
											e.replace)
						}
					} else {
